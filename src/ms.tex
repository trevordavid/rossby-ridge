...
\edit1{Finally, \citet{McQuillan2014} published weight parameters for each star, a metric which was designed to serve as a proxy for reliability, based on a star's autocorrelation function (ACF) properties and its position in \teff–\prot–LPH space, where LPH designates the local peak height of the ACF. Those authors selected $w>0.25$ for an acceptable compromise between real detections and false positives. In Figure~\ref{fig:weights}, we show that the long-period pile-up is apparent even for $w>0.3$, and we conclude that the majority of stars on the long-period pile-up are likely to be genuine detections and not an artifact of some detection threshold.}
