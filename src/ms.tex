...
\begin{figure*}
    ...
    \caption{Color-magnitude diagram. The two panels are identical with exception of the orange points. In the background, a 2d histogram of the Kepler field, where darker values represent a higher density of stars, up to a threshold beyond which the highest density is saturated (shown in white). Shown by orange points are stars with photometric rotation period detections from either the \citet{McQuillan2014} or \citet{Santos2021} catalogs, on the left and right, respectively. The CKS long-period pile-up stars identified in this work are shown by black points. A Gaussian kernel density estimation of the \citet{Hall2021} asteroseismic sample is shown by black contours.}
    \label{fig:cmd}
\end{figure*}
