...
Another possibility is that the long-period pile-up is only present for stars with masses $\gtrsim$~1~\msun, in which case the Sun's period and age require no explanation. However, in this scenario we would have no explanation for why hotter stars pile-up while cooler stars do not, especially given that the pile-up appears to occur at low enough Ro that the cooler stars should appear to pile up as well. As shown in Figure~\ref{fig:xmatch}, it appears that the long-period pile-up does extend to temperatures as low as the Sun's, though this is not apparent from smaller samples such as the CKS sample or the \citet{Hall2021} asteroseismic sample (see also \S\ref{subsec:extent}). Kernel density estimation of the joint \teff--\prot distribution in the \lamostmcq sample, as well as a cross-match between LAMOST and \citet{Santos2021}, also suggests the pile-up exists at \teff as low as 5500~K or even 5000~K. We thus reject the explanation that WMB only operates for $\gtrsim$~1~\msun stars.
