...
\edit1{Given that the long-period pile-up becomes apparent when adopting high-precision \teff from spectroscopic catalogs, it is also natural to consider the high-precision photometry of the Gaia mission \citep{Gaia2016}. While the Gaia photometry are susceptible to reddening, we show in Appendix~\ref{app:gaia} that the long-period pile-up can indeed be recovered in the color–period plane when restricting to stars with low interstellar reddening.}
...
To investigate the origin of this offset, we cross-matched the \citet{Hall2021} sample with the LAMOST DR5 catalog \citep{Xiang2019} \edit1{and the CKS sample}. We found a root mean square (RMS) of the residuals between the \citet{Hall2021} and LAMOST DR5 \teff measurements of 55~K, with a median offset of 41~K, such that the LAMOST \teff scale is cooler, on average (see Appendix~\ref{app:teff}). \edit1{Similarly, we found excellent agreement between the \teff scales of the CKS and \citet{Hall2021} samples, with a median offset of 29~K (such that the CKS scale is hotter) and an RMS of 41~K.}
...
\edit1{\citet{Masuda2021} inferred the \prot distribution of 144 late-F/early-G dwarfs in the Kepler field from their precise stellar radii and spectroscopically determined \vsini. As with the asteroseismic sample, we compare the CKS and \lamostmcq samples with the results of those authors. We find that the long-period pile-up we observe appears to overlap with the \citet{Masuda2021} data. We therefore consider it likely that the long-period pile-up is the same feature, or an extension of the same feature, that was detected in the \citet{Hall2021} and \citet{Masuda2021} studies.}

\begin{figure*}
    \centering
    \includegraphics[width=\linewidth]{./figures/asteroseismic.pdf}
    \caption{Comparison of CKS (left column) and \edit1{\lamostmcq} samples (right column) with the \citet{Hall2021} main-sequence asteroseismic sample (top row) \edit1{and the \citet{Masuda2021} sample (bottom row)} in the \teff--\prot plane. Note, the CKS, LAMOST, and \citet{Hall2021} samples derive \teff from distinct pipelines. A constant offset of +41~K was applied to the LAMOST \teff \edit1{in the upper left panel} to bring that sample onto the same scale as the asteroseismic sample (see Appendix~\ref{app:teff}). Using LAMOST \teff, where it exists, for the \citet{Hall2021} sample brings that sample into even closer agreement with the long-period pile-up in the LAMOST-McQuillan sample.}
...
\edit1{To this point, we have not addressed the detection biases inherent to Kepler rotation period catalogs. As rotation periods become longer, they become more difficult to measure directly from rotational brightness modulations in Kepler time series. This is true for at least three reasons: (1) the Kepler baseline is finite, (2) photometric variation amplitude declines with increasing rotation period, and (3) photometric variations tend to become less periodic with increasing rotation period. This is important because, if a period detection threshold is Ro-dependent and the detection threshold Ro$_\mathrm{thresh}$ is close to the value \rocrit at which WMB becomes important, then the location of the pile-up in the catalogs considered here may depend sensitively on how detectability depends on Ro. For example, if Ro$_\mathrm{thresh}$ is only slightly larger than \rocrit and the long-period pile-up has some non-negligible width in period space (due either to astrophysics or measurement error), then it's possible the current data set only reveals the lower edge of the long-period pile-up.}
...
\edit1{Critically, we also note that while an Ro-dependent detection threshold may reproduce the upper edge of the \teff–\prot distribution, it would not be responsible for producing a pile-up or overdensity at this edge. To our knowledge, there are no systematics of the rotation period catalogs considered here that would bias stars towards the detection threshold. Furthermore, two independent samples with different observational biases than the rotation period catalogs considered here also appear to yield a pile-up of stars at the long-period edge. These samples are the Kepler asteroseismic sample, which measures rotation through asteroseismic mode splitting (discussed in \S\ref{subsec:asteroseismic}), and the sample of Kepler stars with \vsini from high-resolution spectroscopy, from which \citet{Masuda2021} inferred the peak of the \teff–\prot distribution using constraints on the stellar radii. We do note, however, that we can not rule out the possibility that the long-period pile-up is wider in period space than we observe, with stars just above the pile-up being undetected through rotational brightness modulations.}
...
Furthermore, we note that there are important differences between the samples considered here and the \citet{Hall2021} asteroseismic sample. The present samples have rotation periods detected from photometric variations and, for a survey of finite baseline and sensitivity like Kepler, photometric rotation periods are harder to detect for slower rotators, smaller amplitude variations, and more stochastic variability patterns. Exacerbating the problem of detection, slower rotation tends to be accompanied by smaller-scale and less periodic variability. The asteroseismic sample, by comparison, is not biased against quiet, unspotted stars and is more likely to contain pile-up stars that our sample may have missed. \edit1{The same argument is true for the \citet{Masuda2021} sample.}
