...
The LAMOST--Kepler project derived homogeneous spectroscopic parameters from low-resolution ($R\sim$~1800) LAMOST DR5 spectra for approximately 40\% of the Kepler field \citep{Zong2018, Xiang2019}. The \citet{Xiang2019} catalog derived stellar parameters using the DD-Payne pipeline, which builds on the method of \citet{Ting2017b} by incorporating elements of the Cannon \citep{Ness2015} and uses the overlap with GALAH DR2 and APOGEE DR14 as training data. We cross-matched the LAMOST--Kepler stellar parameter catalog of \citet{Xiang2019} with the \citet{McQuillan2014} catalog, which published rotation periods for $>$34000 Kepler targets, as well as the \citet{Santos2021} catalog, which provides rotation periods for $>$55000 FGKM stars observed by Kepler.
...
%This is a bit misleading. Stars reach Ro = 2 at very roughly halfway through the MS, and stay there until the MS turnoff. For higher mass stars with MS lifetimes of 4 Gyr, it means that stars on the ridge should have a ~2 Gyr age spread. For stars with 10 Gyr MS lifetimes, it should be something close to a 5 Gyr age spread. 
...
%JVS: I am not sure this is actually what you expect to see. If you go back and look at the Lanzafame and Spada paper from 2015, the spin down never stalls out entirely... there is always evolution in the period, even though it is slower than you would have predicted with a solid body model. In the solar mass stars, you never see something as stark as the Praesepe/NGC6811 overlap. You're also working in a regime where initial conditions, saturated spin down, and cluster age uncertainties can really get you. I'm not sure you can attribute the overlap in Fritzewski to core-envelope decoupling... it could also just be you are looking at clusters that are coeval within the age uncertainties (remember, you're dealing with isochrone ages that come from stars with convective cores at these ages, and they have *nasty* systematic shifts depending on what you assumed about the degree of convective core overshooting).  I would tread carefully here. 
...
%The physical mechanism responsible for WMB is not known, but one hypothesis is that the efficiency of magnetic braking declines with magnetic field complexity \citep[e.g.][]{Reville2015, vanSaders2016, Garraffo2016, Metcalfe2016, Metcalfe2019}.
...
%JVS: I am confused by this picture. Why would the slow in apparent spin down make the photometric variability drop? The variability doesn't care about the spin-down rate, it cares about the instantaneous rotation (and maybe differential rotation). In this picture you are still stuck without a reason for why a more slowly rotating star should be more active than a more rapidly rotating one. The information you add here doesn't allow you to support or reject the speculation in Gordon 2021, so I would vote to make this speculation more modest, saying that the gap is perhaps an epoch of rapid spin down and leave it at that.
